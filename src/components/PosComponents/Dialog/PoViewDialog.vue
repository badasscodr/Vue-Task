<template>
    <v-dialog v-model="dialogView" max-width="880px" content-class="po-view-dialog" :retain-focus="false" @click:outside="closeView">
                ...
                <div class="view-table-wrapper">
                    <v-data-table
                        ...
                        <template v-slot:[`item.description`]="{ item }">
                            <p class="mb-0">{{ item.description !== '' && item.description !== null ? item.description : '--' }}</p>
                        </template> 
                    </v-data-table>
                </div>
...
export default {
    ...
    methods: {
        ...
        showDeleteInnerDialog() {
            this.$emit('deleteOpen', this.editedItem)
        },
        tax() {
        ...
        getWarehouseAddress(id) {
            if (typeof this.getWarehouse !== 'undefined' && this.getWarehouse !== null &&
                ...
                this.getWarehouse.results !== 'undefined' && this.getWarehouse.results !== null &&
...
@import '../../../assets/scss/pages_scss/po/poViewDialog.scss';
</style>
